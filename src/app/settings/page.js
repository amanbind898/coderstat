    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg p-4 border border-gray-200 hover:border-blue-300 transition-all">
          ...
          <input
            ...
            className={`w-full p-3 border-2 rounded-lg focus:ring-2 focus:ring-blue-700 focus:border-blue-700 transition-all ${
      ...
      <div className="bg-white rounded-lg p-4 border border-gray-200 hover:border-blue-300 transition-all">
        ...
        <textarea
          ...
          rows={4}
          maxLength={500}
          className={`w-full p-3 border-2 rounded-lg focus:ring-2 focus:ring-blue-700 focus:border-blue-700 transition-all resize-none ${
            isEditing ? 'bg-white border-gray-300' : 'bg-gray-50 border-gray-200'
          }`}
          disabled={!isEditing}
        />
        ...
        {socialLinks.map(({ name, icon: Icon, label, placeholder }) => (
          <div key={name} className="bg-white rounded-lg p-4 border border-gray-200 hover:border-purple-300 transition-all">
            ...
            <input
              ...
              className={`w-full p-3 border-2 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all ${
                isEditing ? 'bg-white border-gray-300' : 'bg-gray-50 border-gray-200'
              }`}
              disabled={!isEditing}
            />
        ...
        {codingPlatforms.map(({ name, label, icon, placeholder }) => (
          <div key={name} className="bg-white rounded-lg p-4 border border-gray-200 hover:border-green-300 transition-all">
            ...
            <input
              ...
              onChange={handleInputChange}
              placeholder={placeholder}
              className={`w-full p-3 border-2 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all ${
                isEditing ? 'bg-white border-gray-300' : 'bg-gray-50 border-gray-200'
              }`}
              disabled={!isEditing}
            />
              ...
              <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
                ...
                <div className="flex-1 text-center sm:text-left">
                  <h2 className="text-2xl font-bold text-gray-900 mb-1">
                    {isLoading ? <Skeleton width={180} /> : user?.fullName}
                  </h2>
                  <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4 text-sm text-gray-600 mb-3">
                    <div className="flex items-center gap-2">
                      <Mail className="w-4 h-4" />
                      <span>{isLoading ? <Skeleton width={150} /> : user?.primaryEmailAddress?.emailAddress}</span>
                    </div>
